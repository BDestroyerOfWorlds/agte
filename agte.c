...
char *load_file(const char *path, int *out_chr_cnt) {
...
bool cap_enough(char **buffer, size_t *current_cap, size_t needed_cap) {
...
void get_cursor_coordinates(const char *buffer, int cursor_posi, int *out_line,
                            int *out_col) {
  int line = 0;
  int col = 0;
...
int main(int argc, char *argv[]) // I need to refactor this whole thing tbh...
{
  ...
  while (!WindowShouldClose()) {
    ...
    if (caps_helper == KEY_CAPS_LOCK) /* this is a really bad solution becuase
    ...
    Rectangle content = {
        0, 0, fmaxf(panel.width, 32 + max_line_len * (char_width + 0.5F)),
        fmaxf(707, (line_count * 22) + 22)};

    GuiScrollPanel(panel, NULL, content, &scroll, &view);
    BeginScissorMode(view.x, view.y, view.width, view.height);

    DrawTextEx(Lilex, buffer, (Vector2){32 + scroll.x, 16 + scroll.y}, 20, 1,
               BETTER_WHITE);
    ...
    DrawTextEx(icons, saved_icon_text, (Vector2){1225, 16}, saved_icon_size, 1,
